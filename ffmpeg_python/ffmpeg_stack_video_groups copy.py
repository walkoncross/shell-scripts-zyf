...
from ffmpeg_python_tools import get_video_groups, stack_two_videos_with_audio_alignment


def stack_video_list_in_pairs(video_list,
                     save_dir='./', vstack=False,
                     try_align=True, verbose=False):
...
def stack_video_groups(video_group_list, save_dir='./', vstack=False, verbose=False):
    for group in video_group_list:
        if len(group) > 1:
            print('===> stack group: ', group)
            stack_video_list_in_pairs(group, save_dir, vstack, verbose=verbose)
...
def list_and_stack_video_groups(input_dir, save_dir='', suffixes='', 
    ...
    stack_video_groups(group_list, save_dir, vstack, verbose)
...
def _make_argparser():
    ...
    parser.add_argument('-v', '--verbose',
                        ...
                        action='store_true',
                        default=False,
                        help='If set, print verbose infomation')
...
if __name__ == '__main__':
    ...
    list_and_stack_video_groups(
        ...
        args.suffixes, args.delimiter, 
        args.vstack, args.verbose)
