...
ffmpeg -hide_banner -i $1 -i $2 -filter_complex '[0:v] crop=h=(ih/2)*2:w=(iw/2)*2 [v0_crop]; [1:v] crop=h=(ih/2)*2:w=(iw/2)*2 [v1_crop]; [v0_crop][v1_crop] vstack' ${output_video}
else
...
ffmpeg -hide_banner -i $1 -i $2 	\
	-filter_complex '[0:v] crop=h=(ih/2)*2:w=(iw/2)*2 [v0_crop]; [1:v] crop=h=(ih/2)*2:w=(iw/2)*2 [v1_crop]; [v1_crop][v0_crop] scale2ref=w=-2:h=ih [rightvid][leftvid]; [leftvid][rightvid] vstack [outvid]' \
	-map '[outvid]' \
	-map 0:a \
	${output_video}
fi
