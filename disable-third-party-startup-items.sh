...
echo "Disabling system LaunchDaemons..."
for label in \
...
echo "Disabling user LaunchAgents..."
for label in \
  CGEData.user.agent \
...
do
  launchctl bootout "gui/$uid" "/Library/LaunchAgents/$label.plist" 2>/dev/null || true
  launchctl bootout "gui/$uid/$label" 2>/dev/null || true
  launchctl disable "gui/$uid/$label" 2>/dev/null || true
done

echo "Checking remaining loaded third-party items..."
